import collections, re, socket, sys, threading, time, urllib, urllib.request, os
import xbmc, xbmcaddon, xbmcgui, xbmcplugin, xbmcvfs
import resources.lib.asciidamnit as asciidamit, resources.lib.musicbrainzngs as musicbrainzngs, resources.lib.mypithos as mypithos
...
iconart = xbmc.vfstranslatePath(os.path.join('special://home/addons/plugin.audio.pandoki',  'icon.png'))
...
#        xbmcvfs.delete(song['path_lib'])
    ...
    def Path(self, s):
        ...
        s['path_cch'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s - %s.%s"            % (Val('cache'), s['artist'], s['title'],  s['encoding'])))
        s['path_dir'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s/%s - %s"            % (lib,          s['artist'], s['artist'], s['album'])))
        s['path_m4a'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s/%s - %s/%s - %s.%s" % (lib,          s['artist'], s['artist'], s['album'], s['artist'], s['title'], 'm4a'))) #s['encoding'])))
        s['path_mp3'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s/%s - %s/%s - %s.%s" % (lib,          s['artist'], s['artist'], s['album'], s['artist'], s['title'], 'mp3'))) #s['encoding'])))
        s['path_lib'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s/%s - %s/%s - %s.%s" % (lib,          s['artist'], s['artist'], s['album'], s['artist'], s['title'], s['encoding'])))
        s['path_alb'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s/%s - %s/folder.jpg" % (lib,          s['artist'], s['artist'], s['album'])))
        s['path_art'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s/folder.jpg"         % (lib,          s['artist']))) #.decode("utf-8")

        title = ''.join(c for c in self.station['title'] if c not in badc)
        s['path_m3u'] = xbmcvfs.translatePath(asciidamnit.asciiDammit("%s/%s.m3u"                % (lib, title)))
        s['path_rel'] = xbmcvfs.translatePath(asciidamnit.asciiDammit(   "%s/%s - %s/%s - %s.%s" % (     s['artist'], s['artist'], s['album'], s['artist'], s['title'], s['encoding'])))
...
#        self.Auth()
    ...
    def Loop(self):
        ...
        while (not xbmc.Monitor().abortRequested) and (not self.abort) and (self.once or self.player.isPlayingAudio()):
            time.sleep(0.01)
        ...
        Log('Pankodi Exiting XBMCAbort?=%s PandokiAbort?=%s ' % (xbmc.abortRequested, self.abort), None, level = xbmc.LOGINFO)
        Prop('run', '0')
